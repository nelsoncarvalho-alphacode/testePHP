<?php
session_start();
include("seguranca.php");
include("conexao.php");
include("centrodecusto.php");
include("centrodecustodao.php");

$conexao = new Conexao();
$conexao->abrirConexao();

if(!empty($_POST["idcentrodecusto"])){
	$centroDeCusto = new CentroDeCusto();
	$centroDeCustoDao = new CentroDeCustoDAO($conexao);
	$centroDeCusto->setIdCentroDeCusto($_POST["idcentrodecusto"]);
	$query = $centroDeCustoDao->consultarCentroDeCusto($centroDeCusto);
	$resultset = $centroDeCustoDao->listarCentroDeCusto($query);
}
?>
<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>Administração</title>
	
	<link href="bootstrap-4.1.3/css/bootstrap.min.css" rel="stylesheet">
	<script src="jquery/jquery-3.1.1.min.js"></script>
    <script src="popper/popper.min.js"></script>
	<script src="bootstrap-4.1.3/js/bootstrap.min.js"></script>
	
	<script src="js/jquery.mask.min.js"></script>
	<script src="js/funcoes.js"></script>
	<script src="js/menu.js"></script>
  
	<!-- Custom styles for this template -->
	<link href="css/padding.css" rel="stylesheet">
    <link href="css/scroll.css" rel="stylesheet">
	<link href="css/navbar.css" rel="stylesheet">
	<link href="css/sticky-footer-navbar.css" rel="stylesheet">
	<script>
	 $(document).ready(function() {
		$('#cnpj').mask('00.000.000/0000-00', {reverse: true});
		$('#cep').mask('00000-000');
		$('#telefone').mask('(00) 00000-0000');
		$('#msgvalidarcnpj').hide();
		$('#msgvalidardigitocnpj').hide();
		$(window).keydown(function(event){
			if(event.keyCode == 13) {
			  event.preventDefault();
			  return false;
			}
		});	
		
		$('#centrodecusto').keydown(function(event){
		   if(event.keyCode == 13){
			$('#razaosocial').focus();
		   }
		});
		
		$('#razaosocial').keydown(function(event){
		   if(event.keyCode == 13){
			$('#cnpj').focus();
		   }
		});
		
		$('#cnpj').keydown(function(event){
		   if(event.keyCode == 13){
			$('#endereco').focus();
		   }
		});
		
		$('#endereco').keydown(function(event){
		  if(event.keyCode == 13){
			$('#numero').focus();
		  }
		});
		
		$('#numero').keydown(function(event){
		  if(event.keyCode == 13){
			$('#bairro').focus();
		  }
		});
		
		$('#bairro').keydown(function(event){
		  if(event.keyCode == 13){
			$('#cep').focus();
		  }
		});
		
		$('#cep').keydown(function(event){
		  if(event.keyCode == 13){
			$('#idpais').focus();
		  }
		});
		
		$('#idpais').keydown(function(event){
		  if(event.keyCode == 13){
			$('#idestado').focus();
		  }
		}); 
		 
		$('#idestado').keydown(function(event){
		  if(event.keyCode == 13){
			$('#idcidade').focus();
		  }
		});
		
		$('#idcidade').keydown(function(event){
		  if(event.keyCode == 13){
			$('#email').focus();
		  }
		});
		
		$('#email').keydown(function(event){
		  if(event.keyCode == 13){
			$('#telefone').focus();
		  }
		});
		
		$('#telefone').keydown(function(event){
		  if(event.keyCode == 13){
			$('#habilitarboleto').focus();
			$("#habilitarboleto").prop("checked", true);
		  }
		});
		
		$('input:radio[name=habilitarboleto]').keydown(function(e){
			var code = e.keyCode || e.which;
			if(code==13){
				$('#obs').focus();
			}
		});
		 
		$('#obs').keydown(function(event){
		  if(event.keyCode == 13){
			$("#gravar").trigger("click");
		  }
		}); 
		
		$("#voltar").click(function() {
		   formCentroDeCusto.action = "centrodecustolista.php";
		   formCentroDeCusto.submit();
		});
		
		 //combo
       $("#idpais").change(function(){
           var idpais = $(this).val();
		   $("#idestado").empty();
		   $("#idestado").append("<option selected value=''>Selecione um estado</option>");
		   $("#idcidade").empty();
		   $("#idcidade").append("<option selected value=''>Selecione uma cidade</option>");
		   $.ajax({
                url: 'estadocontroller.php',
                type: 'post',
                data: {acao:"listarestado",idpais:idpais},
                dataType: 'json',
                success:function(response){
					var len = response.length;
					for( var i = 0; i<len; i++){
					    var idestado = response[i]['idestado'];
						var sigla = response[i]['sigla'];
						var estado = response[i]['estado'];
						$("#idestado").append("<option value='"+idestado+"'>"+sigla+"-"+estado+"</option>");
                    }
				}
            });
       });	 
		
	   //combo
       $("#idestado").change(function(){
            var idestado = $(this).val();
		    $("#idcidade").empty();
		    $("#idcidade").append("<option selected value=''>Selecione uma cidade</option>");
		    $.ajax({
                url: 'cidadecontroller.php',
                type: 'post',
                data: {acao:"listarcidade",idestado:idestado},
                dataType: 'json',
                success:function(response){
					var len = response.length;
					for( var i = 0; i<len; i++){
                        var idcidade = response[i]['idcidade'];
						var cidade = response[i]['cidade'];
						$("#idcidade").append("<option value='"+idcidade+"'>"+cidade+"</option>");
                    }
                }
            });
        });
  
		$("#formCentroDeCusto").submit(function(){
			$('#msgvalidarcnpj').hide();
			$('#msgvalidardigitocnpj').hide();
			if($('#cnpj').val()!=''){
				if (!validarCNPJ($('#cnpj').val())){
					$('#msgvalidardigitocnpj').show();
					$('#cnpj').focus();
					return false;
				}
			}
			
			if($('#cnpj').val()!=""){
				$.ajax({
					url: 'centrodecustocontroller.php',
					type: 'post',
					data: {
						acao: "listarcentrodecustodocumentos",
						idcentrodecusto: $('#idcentrodecusto').val(),
						cnpj: $('#cnpj').val()
					},
					success:function(result){
						if(result==1){
							$('#msgvalidarcnpj').show();
							$('#cnpj').focus();
						}else{
							formCentroDeCusto.acao.value = "gravar";
							formCentroDeCusto.action = "centrodecustocontroller.php";
							formCentroDeCusto.submit();
						}
					}
				});
				return false;
			}
        });
	 });
	</script>	
  </head>
  <body>
  
    <header>      
     <?php include("menu.php");?>
    </header>
	
    <main role="main">

		<div id="cadastro" class="top col">
			<form id="formCentroDeCusto" name="formCentroDeCusto" method="post" action="centrodecustocontroller.php">
				<input type="hidden" id="acao" name="acao" value="gravar">
				<input type="hidden" id="idcentrodecusto" name="idcentrodecusto" value="<?php echo isset($resultset["idcentrodecusto"])? $resultset["idcentrodecusto"]:"";?>" >
		        <div class="card">
					<h5 class="card-header">Cadastro de centro de custo:</h5>
					<div class="card-body">
						<div class="form-group">			
							<label for="centrodecusto"><font color="red">*</font>Centro de custo:</label>
							<input type="text" id="centrodecusto" name="centrodecusto" class="form-control" value="<?php echo isset($resultset["centrodecusto"])? $resultset["centrodecusto"]:"";?>" required autofocus>
						</div>						
						<div class="form-group">
							<label for="endereco">Endereço:</label>
							<input type="text" id="endereco" name="endereco" class="form-control" value="<?php echo isset($resultset["endereco"])? $resultset["endereco"]:"";?>">
						</div>
						<div class="form-group row">
							<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
								<label for="numero">Nº:</label>
								<input type="text" id="numero" name="numero" class="form-control" value="<?php echo isset($resultset["numero"])? $resultset["numero"]:"";?>">
						    </div>
							<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
								<label for="bairro">Bairro:</label>
								<input type="text" id="bairro" name="bairro" class="form-control" value="<?php echo isset($resultset["bairro"])? $resultset["bairro"]:"";?>">
							</div>
							<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
								<label for="cep">Cep:</label>
								<input type="text" id="cep" name="cep" class="form-control" value="<?php echo isset($resultset["cep"])? $resultset["cep"]:"";?>">
						    </div>
						</div>
						<div class="form-group">
							<label for="email"><font color="red">*</font>Email:</label>
							<input type="email" id="email" name="email" class="form-control" value="<?php echo isset($resultset["email"])? $resultset["email"]:"";?>" required>
						</div>						
	                    <div class="form-group row">
							<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
								<label for="telefone">Telefone:</label>
								<input type="tel" id="telefone" name="telefone" class="form-control" value="<?php echo isset($resultset["telefone"])? $resultset["telefone"]:"";?>">
							</div>
						</div>
						<div class="form-group">
						    <button class="btn btn-outline-secondary" type="submit" id="gravar" name="gravar">Gravar</button>
							<button class="btn btn-outline-secondary" type="button" id="voltar" name="voltar" >Voltar</button>									
						</div>							
					</div>
				</div>
			</form>		
		</div>
 
     <?php include("footer.php");?>
	  
    </main>
  
  </body>
</html>
<?php 
$conexao->fecharConexao();
?>
...
<div class="row">
    <div class="col-md-8 offset-md-2">
        <h1 class="text-center mb-4">Criar Pedido</h1>
        <form action="{{ route('purchase_orders.store') }}" method="POST">
            @csrf
            <div class="mb-3">
                <label for="client_name" class="form-label">Cliente:</label>
                <span class="text-danger">*</span>
                <input type="text" class="form-control" id="client_name" name="client_name" value="{{ old('client_name') }}" onkeyup="searchClients()" required>
                <input type="text" class="form-control" id="client_id" name="client_id" hidden>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="category" name="category" onchange="filterProducts(this.value)" required>
            ...
            <input type="hidden" id="product_list_data" name="product_list_data" value="">
            <div class="mt-5 mb-3" id="product_add_group">
                ...
                <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3" id="product_list">
                    <!-- Aqui serão adicionados os cards de produtos -->
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    <input type="text" class="form-control" id="total" name="total" readonly required value="{{ old('total') }}">
                    <span class="input-group-text" id="discount-text" style="{{ (old('total')) ? '' : 'display: none;'}}">(5% de desconto para Pix)</span>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <span class="text-danger">*</span>
                <input type="date" class="form-control" id="order_date" name="order_date" required value="{{ old('order_date') }}">
            </div>

            <div class="mb-3">
                <label for="payment_method" class="form-label">Forma de Pagamento:</label>
                <span class="text-danger">*</span>
                <select class="form-select" id="payment_method" name="payment_method" onchange="paymentMethod(this.value)" disabled required>
                    <option value="" {{ (old('payment_method')) ? '' : 'selected disabled'}}>Selecione uma opção</option>
                    ...
                    <option value="pix" {{ old('payment_method') == 'pix' ? 'selected' : '' }}>Pix <span>(5% de desconto)</span></option>
                </select>
            </div>
        ...
        <div class="col">
            <div class="card h-100">
                <div class="card-body d-flex flex-column">
                    ...
                    <div class="mt-auto mx-auto">
                        <button type="button" class="btn btn-primary" onclick="addProduct('${product.id}')">Adicionar ao Carrinho</button>
                    </div>
                </div>
            </div>
        </div>
...
function addProduct(productId, maxQuantity) {
    // Obter informações do produto selecionado
    let productName = document.getElementById('product_' + productId).textContent;
    let quantity = parseInt(document.getElementById('quantity_' + productId).value);
    let price = parseFloat(document.getElementById('price_' + productId).textContent);
